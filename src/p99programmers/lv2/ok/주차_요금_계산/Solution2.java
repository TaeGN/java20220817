	...
	private static int[] sol(int[] fees, String[] records) {
		...
		for(String record : records) {
			...
			if(park.containsKey(carNum)) {
				...
				if(parkFee.containsKey(carNum)) {
					parkFee.replace(carNum, parkFee.get(carNum) + pTime);
				} else {
					parkFee.put(carNum, pTime);
				}
				park.remove(carNum);
			} else {
		...
		for(Map.Entry<String, String> entry : park.entrySet()) {
			...
			if(parkFee.containsKey(entry.getKey())) {
				parkFee.replace(entry.getKey(), parkFee.get(entry.getKey()) + pTime);
			} else {
				parkFee.put(entry.getKey(), pTime);
			}
 		}
		for(Map.Entry<String, Integer> entry : parkFee.entrySet()) {
			parkFee.replace(entry.getKey(), Fee(entry.getValue(), fees));
		}
